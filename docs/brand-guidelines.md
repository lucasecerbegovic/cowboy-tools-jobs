...
| Header | height `64px`, sticky, `border-bottom: 1px solid #000`, white; brand is logo mark (`/cowboy-tools-logo-full.png`, 32px) + `SITE_NAME` mono |
...
Match cowboytools.ca: `/favicon.ico` (32×32) and apple touch `/icon-192.png`. Wired via `metadata.icons` in `app/layout.tsx`.

### Footer

Black. Four columns: brand + locality, For Tradespeople, For Employers, Company. Brand column uses the logo mark (40px) + `SITE_NAME`, then locality, then a sentence-case CTA to `https://cowboytools.ca` (“Need great deals on tools?”). The employer directory is parked, so footer links go to jobs / post / about — not `/employers`. Company → About points at `/about`. Labels mono 11px `0.18em` at `0.6`. Links Geist 14px, hover opacity `0.7`. No job-alert signup in the footer.
