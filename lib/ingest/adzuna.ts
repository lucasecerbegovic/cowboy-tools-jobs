...
function mapAdzunaJob(job: AdzunaJob, trade: TradeSlug): NormalizedJob | null {
...
export async function ingestAdzuna(): Promise<IngestResult> {
  ...
  try {
    for (const trade of TRADE_SLUGS) {
      ...
      if (!response.ok) {
        console.log(
          `Adzuna ${what} failed: ${response.status} ${await response.text()}`,
        );
        continue;
      }
      const payload = (await response.json()) as AdzunaSearchResponse;
      for (const result of payload.results ?? []) {
        const mapped = mapAdzunaJob(result, trade);
