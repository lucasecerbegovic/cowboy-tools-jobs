...
import { ingestUsaJobs } from "@/lib/ingest/usajobs";
import { recordIngestRun } from "@/lib/ingest/persist";
import type { IngestResult } from "@/lib/ingest/types";

const SOURCES = ["job_bank", "adzuna", "usajobs"] as const;
...
async function runOne(
  ...
  switch (source) {
    case "job_bank":
      result = await ingestJobBank({ live: liveJobBank });
      break;
    case "adzuna":
      result = await ingestAdzuna();
      break;
    case "usajobs":
      result = await ingestUsaJobs();
      break;
    default: {
...
export async function runIngest(options?: {
  ...
  if (requested !== "all" && !isIngestSource(requested)) {
    return [
      {
        source: "all",
        error: `Unknown source "${requested}". Use job_bank, adzuna, usajobs, or all.`,
      },
    ];
  }
  ...
  for (const source of sources) {
    results.push(await runOne(source, options?.liveJobBank));
  }
  return results;
}
